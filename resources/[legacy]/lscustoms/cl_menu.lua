...
local function SetRightLabel(item, owned, id, type, section)
	local label
	if DebugMode then
		label = id
	elseif owned then
		...
		if type == "mod" then
			label = "~g~$"..Config.Mods[section].Price
		else
			label = "~g~$0"
		end
	end
...
function Menu:CreateCustomization()
	...
	for modType = 0, 49 do
		...
		if modConfig and numMods > 0 and (not modConfig.Condition or modConfig.Condition(Vehicle)) and (not modConfig.DebugOnly or DebugMode) then
			...
			if modType == 23 then
				for modSubType = 0, 12 do
					local modSubTypeConfig = modConfig.Types[modSubType]

					if modSubTypeConfig and (not modSubTypeConfig.Condition or modSubTypeConfig.Condition(Vehicle)) and (not modSubTypeConfig.DebugOnly or DebugMode) then
						local modSubTypeMenu = self.menuPool:AddSubMenu(modTypeMenu.SubMenu, modSubTypeConfig.Name, "", true, true)
						modSubTypeMenu.SubMenu:SetMenuWidthOffset(Config.Menu.WidthOffset)
						modSubTypeMenu.SubMenu.Settings = menu.Settings

						modSubTypeMenu.SubMenu.OnMenuClosed = function(_menu)
							RestorePreview()
						end

						modSubTypeMenu.Item.Activated = function(_menu, _item)
							modSubTypeMenu.SubMenu:Clear()

							local oldWheelType = GetVehicleWheelType(Vehicle)
							SetVehicleWheelType(Vehicle, modSubType)
							local mod = GetVehicleMod(Vehicle, modType)
							local numWheelMods = GetNumVehicleMods(Vehicle, modType)

							for modIndex = -1, numWheelMods - 1 do
								local modName
								if modIndex == -1 then
									modName = "Stock"
								...
								local modEnabled = modIndex == mod and oldWheelType == modSubType
								SetRightLabel(modItem, modEnabled, modIndex, customType, modType)
								if modEnabled then
								...
								modItem.Activated = function(_menu, _item)
									...
									VehicleInfo.components[4] = modSubType
								...
								modSubTypeMenu.SubMenu:AddItem(modItem)
							end
						end
					end
				end
	...
	lightsMenu.Item.Activated = function(_menu, _item)
		...
		for i = 0, 3 do
			...
			neonItem.CheckboxEvent = function(_menu, _item, _value)
				...
				VehicleInfo.components[5][tostring(i)] = value
			end
