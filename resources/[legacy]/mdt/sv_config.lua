			...
			Add = function(source, authorId, targetId, data)
				...
				if data.charges then
					for _, v in ipairs(data.charges) do
						if type(v.id) == "number" and (v.amount == nil or type(v.amount) == "number") then
							charges[tostring(v.id)] = v.amount or 1
						end
					end
				end
								...
								if chargeInfo then
									if chargeInfo.time then
										...
										elseif minutes ~= -1 then
											minutes = minutes + chargeInfo.time * (charge.amount or 1)
										end
									end
									if chargeInfo.fine then
										fine = fine + chargeInfo.fine * (charge.amount or 1)
									end
								end
							end
							if minutes ~= 0 then
