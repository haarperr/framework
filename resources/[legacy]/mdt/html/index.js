...
window.addEventListener("message", function(event) {
	...
	if (data.charges) {
		...
		for (var charge of Charges) {
			if (!charge.active) { charge.charge += " (Deprecated)" }
		...
		entry.onclick = function(e) {
			...
			if(charge.active) {
				...
				if (isRoot) {
					...
					e.target.querySelector("#charge-count").innerHTML = "1x";
				} else {
...
function filterEntries(rootId, filter, check) {
	...
	for (var item of items) {
		...
		var enabled = (check != undefined ? check(item) : true) && (filter == "" || filter == undefined || text.toLowerCase().includes(filter.toLowerCase())) && (item.charge.active);
