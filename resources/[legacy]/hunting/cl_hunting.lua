	...
	while true do
		...
		if zone then
			...
			if count < zone.Max / playerCount then
				...
				local spawnZone = GetNameOfZone(spawnCoords.x, spawnCoords.y, spawnCoords.z)
				local spawnInterior = GetInteriorAtCoords(spawnCoords.x, spawnCoords.y, spawnCoords.z)

				if retval and spawnInterior == 0 and not (Config.Zones[spawnZone] and Config.Zones[spawnZone].Nerfed and safeCoords.z >= Config.Zones[spawnZone].minZ and safeCoords.z <= Config.Zones[spawnZone].maxZ) then
...
Citizen.CreateThread(function()
	while true do
		...
		if DoesEntityExist(Target) and exports.oldutils:DrawContext("Harvest", GetEntityCoords(Target)) then
			...
			if checkTarget() then
				local animalModel = GetEntityModel(Target)
				exports.mythic_progbar:Progress(Config.Action, function(wasCancelled)
					if wasCancelled or not checkTarget() then return end
					local damaged = HasEntityBeenDamagedByAnyVehicle(Target) or (Config.Animals[animalModel].DamageType == "Firearm" and HasPedBeenDamagedByWeapon(Target, 0, 1)) or (Config.Animals[animalModel].DamageType == "Knife" and HasPedBeenDamagedByWeapon(Target, 0, 2) and (not HasPedBeenDamagedByWeapon(Target, -1716189206, 0) and not HasPedBeenDamagedByWeapon(Target, -1716189206, 0) and not HasPedBeenDamagedByWeapon(Target, -581044007, 0)))
					
					TriggerServerEvent("hunting:harvest", PedToNet(Target), GetEntityModel(Target), damaged)
				end)
			end
		end
