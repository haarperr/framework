...
function Jail(source, target, minutes, _type)
	...
	exports.GHMattiMySQL:QueryAsync("CALL jail_add(@target, @source, @minutes, @type)", {
		["@target"] = targetId,
		["@source"] = sourceId,
		["@minutes"] = minutes,
		["@type"] = _type,
	...
	local skipSpawn = minutes >= 15 and exports.jobs:CountActiveDuty("doc") >= 2
...
function Unjail(source, keepPosition)
	...
	exports.GHMattiMySQL:QueryAsync("UPDATE jail_active SET end_time=SYSDATE() WHERE character_id=@character_id", {
		["@character_id"] = id
	})
	Jailed[source] = nil
...
function AddTime(source, minutes)
	...
	exports.GHMattiMySQL:QueryAsync("UPDATE jail_active SET end_time=end_time + INTERVAL @minutes MINUTE WHERE character_id=@character_id", {
		["@minutes"] = minutes,
		["@character_id"] = id,
	})
