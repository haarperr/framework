...
local function HandleClosestTv()
    while true do
        ...
        for id, data in pairs(tvs) do
            local tv = GetClosestObjectOfType(data.coords.x, data.coords.y, data.coords.z, 5.0, data.model, false, false, false)
...
local function DrawTv()
    while true do
        ...
        if tvs[id] then
            local tvData = tvs[id]
            local tv = GetClosestObjectOfType(tvData.coords.x, tvData.coords.y, tvData.coords.z, 5.0, tvData.model, false, false, false)
            ...
            while DoesEntityExist(tv) and tvs[id] and closestTv == id do
                ...
                if tvData.draw == "scaleform" then
                    local offset = GetOffsetFromEntityInWorldCoords(tv, tvData.offset.x, tvData.offset.y, tvData.offset.z)
...
local function NearbyTip()
    while true do
        ...
        for _, data in pairs(Config.Objects) do
            local tv = GetClosestObjectOfType(plyCoords.x, plyCoords.y, plyCoords.z, 5.0, data.Model, false, false, false)
            if DoesEntityExist(tv) and #(plyCoords - GetEntityCoords(tv)) <= 2.0 then
                AddTextEntry(GetCurrentResourceName(), Config.NearbyTip)
