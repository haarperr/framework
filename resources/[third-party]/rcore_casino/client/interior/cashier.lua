...
function Cashier_RequestVIP()
    ...
    Cashier_OnQuit()
    local message = string.format(Translation.CASHIER_VIP_CONFIRM_MSG, FormatPrice(Config.CASHIER_VIP_PRICE), FormatTimestamp(Config.CASHIER_VIP_DURATION))
    FullscreenPrompt(Translation.CASHIER_VIP_CONFIRM_CAPT, message, function(yes)
        if yes then
...
AddEventHandler("Casino:TradeResults", function(chipsNow, moneyNow, comment, moneyLimit, pCache)
    if chipsNow and chipsNow ~= -1 then
        ...
        FullscreenPrompt(Translation.CASHIER_TRADEIN_FAILED_CAPT, Translation.CASHIER_TRADEIN_FAILED_MSG, nil, string.format(Translation.CASHIER_SOCIETY_BALANCE, FormatPrice(societyMoney)))
