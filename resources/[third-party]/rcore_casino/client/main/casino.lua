...
local function CasinoGarage_ReplaceCallback(state)
    ...
    if state == -1 then
        ...
        if state == 1 then
            errorMessage = Translation.BOSS_REPLACE_NOJOB
        elseif state == 2 then
            errorMessage = Translation.BOSS_REPLACE_OWNERSHIP_ERROR
        else
            errorMessage = "Unknown Error"
        end
        FullscreenPrompt(Translation.CAPT_ERROR, errorMessage)
    end
    IN_GARAGE = false
end
...
function CasinoGarage_OnInteraction()
    ...
    FullscreenPrompt(Translation.BOSS_REPLACE_CAPTION, Translation.BOSS_REPLACE_QUESTION, function(yes)
        if yes then
...
AddEventHandler("Casino:BecomeVIP", function(playerId, vipUntil)
    if playerId == GetMyPlayerId() then
        ...
        Cashier_OnBecomeVIP()
        ShowFullscreenBonusNotify(Translation.CASHIER_FULLSCREEN_PURCHASE_VIP,
            Translation.CASHIER_FULLSCREEN_PURCHASE_VIP_2, "", "HUD_COLOUR_WAYPOINTDARK")
    end
end)
