...
function AddText(str)
	local Characters = GetCharacterCount(str)
	...
	if Characters < 100 then
		...
		for Index = 0, StringsNeeded do
			AddTextComponentSubstringPlayerName(str:sub(Index * 99, (Index * 99) + 99))
		end
	end
end
...
--end
function FormatXWYH(Value, Value2)
	return (tonumber(Value) or 0)/1920, (tonumber(Value2) or 0)/1080
end

-- Converts normalized position to pixel position
function ConvertToPixel(x, y)
	return math.round(x * 1920), math.round(y * 1080)
end
...
function IsMouseInBounds(X, Y, Width, Height, DrawOffset)
	local MX, MY = math.round(GetDisabledControlNormal(0, 239) * 1920), math.round(GetDisabledControlNormal(0, 240) * 1080)
	...
	-- print(MX, MY, X, Y, Width, Height, json.encode(DrawOffset))
...
function UIMenuColourPanel:Functions()
	...
	--@Key Process
	if IsDisabledControlJustPressed(0, 24) or (IsDisabledControlJustPressed(0, 174) and not self.DisableArrows) then
		self:GoLeft()
	end
	if IsDisabledControlJustPressed(0, 24) or (IsDisabledControlJustPressed(0, 175) and not self.DisableArrows) then
		self:GoRight()
	end
			...
			if not self.Pressed then
				...
				Citizen.CreateThread(function()
					...
					while IsDisabledControlPressed(0, 24) and IsMouseInBounds(self.Grid.X + 20, self.Grid.Y + 20, self.Grid.Width - 40, self.Grid.Height - 40, DrawOffset) do
						Citizen.Wait(0)
						local CursorX, CursorY = ConvertToPixel(GetDisabledControlNormal(0, 239) - DrawOffset.X, GetDisabledControlNormal(0, 240) - DrawOffset.Y)
...
function UIMenuPercentagePanel:Percentage(Value)
    if tonumber(Value) then
        ...
        local Progress = (math.round((GetDisabledControlNormal(0, 239)) * W)  - DrawOffset.X) - self.ActiveBar.X
...
---@return bool
---@public
function UIMenuPercentagePanel:Hovered()
	local DrawOffset = { X = 0, Y = 0}
	if self.ParentItem:SetParentMenu().Settings.ScaleWithSafezone then
		DrawOffset = self.ParentItem:SetParentMenu().DrawOffset
	end
	return IsMouseInBounds(self.BackgroundBar.X, self.BackgroundBar.Y - 4, self.BackgroundBar.Width, self.BackgroundBar.Height + 8, DrawOffset)
end
			...
			if not self.Pressed then
				...
				Citizen.CreateThread(function()
					...
					while IsDisabledControlPressed(0, 24) and IsMouseInBounds(self.BackgroundBar.X, self.BackgroundBar.Y - 4, self.BackgroundBar.Width, self.BackgroundBar.Height + 8, DrawOffset) do
						Citizen.Wait(0)
						local Progress, ProgressY = ConvertToPixel(GetDisabledControlNormal(0, 239), 0)
						Progress = Progress - DrawOffset.X - self.ActiveBar.X
...
---@return int
---@public
function UIMenu:ItemToIndex(Item)
...
function UIMenu:GoUpOverflow()
	...
	self.ReDraw = true
	if self.Items[self:CurrentSelection()].OnSelected ~= nil then
		self.Items[self:CurrentSelection()]:OnSelected(self, self:CurrentSelection())
	end
end
...
function UIMenu:SelectItem()
	...
	elseif subtype == "UIMenuSliderHeritageItem" then
		...
		local menu = self.Children[self.Items[self:CurrentSelection()]]
		self.OnMenuChanged(self, menu, true)
		if menu.Items[menu:CurrentSelection()].OnSelected ~= nil then
			menu.Items[menu:CurrentSelection()]:OnSelected(menu, menu.Items[menu:CurrentSelection()])
		end
	end
end
...
function UIMenu:Draw()
	...
	if self.Settings.ScaleWithSafezone then
		...
		self.DrawOffset = GetSafeZoneBounds()
	end
	if self.ReDraw then
						...
						if Item:Selected() and Item:Enabled() then
							...
							elseif SubType == "UIMenuProgressItem" then
								if IsMouseInBounds(Item.Bar.X, Item.Bar.Y - 12, Item.Data.Max, Item.Bar.Height + 24, self.DrawOffset) then
									local CursorX, CursorY = ConvertToPixel(GetDisabledControlNormal(0, 239), 0)
									Item:CalculateProgress(CursorX - self.DrawOffset.X)
									self.OnProgressChange(self, Item, Item.Data.Index)
									Item.OnProgressChanged(self, Item, Item.Data.Index)
									if not Item.Pressed then
										Item.Pressed = true
