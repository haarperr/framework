...
function StopSlinging()
	Corner = nil
	TriggerEvent("chat:notify", { text = "No longer slinging...", class = "inform" })
end
...
function CanSellInInteriorGroup(interiorGroup)
	return Config.DisabledInteriorGroups[interiorGroup] ~= true
end

function CanSellInInterior(interiorId)
	return Config.DisabledInteriors[interiorId] ~= true
end
...
AddEventHandler("drugs:sell", function()
	...
	TriggerEvent("chat:notify", { text = ("Sold %sx %s for $%s!"):format(CurrentDrugCount, CurrentDrug, CurrentDrugCount * CurrentPrice), class = "inform" })
...
RegisterCommand("corner", function(source, args, command)
	...
	if GetGameTimer() - LastSling < Config.Slinging.Cooldown * 1000 then
		TriggerEvent("chat:notify", { text = "You're slinging too fast!", class = "error" })
		return
	end
	...
	local interiorGroup = GetInteriorGroupId(interiorId)
	if not zone then
	...
	if #items > 0 and CanSellInInterior(interiorId) and CanSellInInteriorGroup(interiorGroup) then
		CurrentDrug = items[GetRandomIntInRange(1, #items)]
		TriggerEvent("chat:notify", { text = ("Slinging %s..."):format(CurrentDrug:lower()), class = "inform" })
	else
		TriggerEvent("chat:notify", { text = "You can't seem to sling here...", class = "error" })
		return
	end
