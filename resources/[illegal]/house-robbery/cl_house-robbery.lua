			...
			if DoesEntityExist(ped) then
				...
				if not shouldAlert and not settings.alerted then
					...
					if settings.Anim and not IsEntityPlayingAnim(ped, settings.Anim.Dict, settings.Anim.Name, 3) then
						exports.emotes:PlayEmote(settings.Anim, false, ped)
					end
...
AddEventHandler("inventory:use_Lockpick", function(item, slotId)
	...
	if property.character_id or not propertySettings then
		exports.mythic_notify:SendAlert("error", "That property is shut too tight...", 7000)
		return
	end

	local hour = GetClockHours()
	if hour < Config.Time[1] and hour > Config.Time[2] then
		exports.mythic_notify:SendAlert("error", "It's too bright out...", 7000)
		return
	end
