...
function Initialize()
	...
	for _, location in ipairs(Config.Labs.Locations) do
		...
		exports.oldinstances:CreateInstance(location.Instance, {
	...
	for _, instance in ipairs(Config.Instances) do
		exports.oldinstances:CreateInstance(instance.id, instance)
	end
...
function GetControl(zoneId)
	...
	for faction, value in pairs(zone) do
		if value and value > 0.2 and totals[faction] and totals[faction] > Config.Reputation.Infamy and (not highestFaction or value > highestAmount) then
			highestAmount = value
