CREATE TABLE IF NOT EXISTS `shops` (
	...
	`stations` LONGTEXT NULL DEFAULT NULL COLLATE 'utf8mb4_bin',
	PRIMARY KEY (`id`) USING BTREE,
	UNIQUE INDEX `shops_id_unique` (`id`) USING BTREE,
	INDEX `shops_id` (`id`) USING BTREE,
	INDEX `shops_storage` (`storage`) USING BTREE,
	CONSTRAINT `shops_storage` FOREIGN KEY (`storage`) REFERENCES `nonstoprp_dev`.`containers` (`id`) ON UPDATE NO ACTION ON DELETE SET NULL,
	CONSTRAINT `prices` CHECK (json_valid(`prices`)),
	CONSTRAINT `containers` CHECK (json_valid(`containers`)),
	CONSTRAINT `stations` CHECK (json_valid(`stations`))
)
