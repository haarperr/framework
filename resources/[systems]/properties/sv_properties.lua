...
function Enter(source, id, shouldConfirm)
	...
	if exports.instances:DoesInstanceExist(instanceId) then
		exports.instances:JoinInstance(instanceId, source)
	else
		exports.instances:CreateInstance(instanceId, {
	...
	if settings.Storage and not Containers[id] then
		local containerId = exports.inventory:LoadContainer("property_id", id, settings.StorageType).id
...
AddEventHandler("properties:start", function()
	...
	for k, property in ipairs(properties) do
		...
		if settings.Custom then
			...
			if settings.Storage and not Containers[id] then
				local containerId = exports.inventory:LoadContainer("property_id", property.id, settings.StorageType).id
